...
\begin{equation}
    ...
    \varphi_p(A)v \simeq \left\| v \right\|_{2} V_m \varphi_p(H_m) e_1 =: \mathbf{\Phi_{p, m}^{PA}}.
\end{equation}
...
\begin{equation}
    ...
    \varphi_p(A)v \simeq \left\| v \right\|_{2} V_m \varphi_p(A_m) e_1 =: \mathbf{\Phi_{p, m}^{RA}}.
\end{equation}
...
\begin{equation*}
    \left\| \varphi_p(A)v - \varphi_{p, RA}^{(m)}  \right\|_2 \le 2 C \left\| v \right\|_2
    \min_{r_m \in \mathcal{Q}_{m}} \max_{z \in \Sigma} \left\| \varphi_p(z) - r_m(z) \right\|_2
\end{equation*}
with a constanc $C \le 11.08$, assuming that $\varphi_p$ is analytic in the neighbourhood of
...
of $\varphi_p(z)$ in the set $\Sigma$.
\begin{comment}
...
\subsection{Pole Selection for Rational Krylov Approximation}
\todo[color=pink]{
