...
\todo[color=cyan]{Add the Jordan canonical form definition from \cite{higham2008functions}.}
...
\todo[color=pink]{
    \autoref{fig:scalarphifunctionscomplexplane} is added for a better
    understanding of the functions as Daniel suggested.
}
\begin{figure}[h]
    \centering
...
\todo[color=pink]{
    This subsection is added to define the matrix sine and cosine and their
    basic properties.
}
...
\todo[color=pink]{Added this section on pole selection. I present here the AAA and the BRASIL algorithms.}
...
they do not allow for restricting the poles, and work like a black-box regarding the pole selection.

\subsection{Reference Evaluations}
\label{sec:exactevaluation}
\todo[color=pink]{
    Added this section on the computation of reference vectors by the method that Daniel suggested.
    There was no need for the splitting the method based on the matrix size since we can use the
    \texttt{scipy.sparse.linalg.expm\_multiply} function for getting the last column of the matrix
    exponential directly for large sparse matrices.
}
...
\todo[color=pink]{Explained the logic behind the scaling of the matrices}
...
\todo[color=pink]{
    Added this section on the implemented schemes of pole selection using the AAA and the BRASIL algorithms.
    I did some experiments that are not presented on the report to fix these schemes:
}
\todo[color=pink]{
    $\xi_1 = \xi^*$ and $\xi_j = \infty$ for $j \ge 2$
    OR
    $\xi_j= \xi^*$ for $j \ge 1$
}
\todo[color=pink]{Comparing the discretization for AAA with \texttt{np.linspace(-a, -b, a)},
    \texttt{np.linspace(-1000, -b, 1000)} and \texttt{np.geomspace(-a, -b, 1000)}
    for $10^2 \le a \le 10^5$ and $b = 10^{-6}$
}

We use the \texttt{baryrat} library introduced in \cite{hofreither2021BRASIL} which implements both
...
by either the AAA or the BRASIL algorithm and we repeat them enough times to fill the $m$ poles $\xi_1, \dots, \xi_m$. With this
scheme for the pole selection, the RA method is denoted by RA-AAAk or RA-BRASILk where k is replaced by its number. We also consider
...
\todo[color=pink]{Added \autoref{fig:krylovapproximationcputime} which compares the CPU time.}
...
\todo[color=pink]{Added \autoref{fig:rationalkrylovpoleselection} which compares the pole selection of RA.}
\todo{Comment on \autoref{fig:rationalkrylovpoleselection}.}
\begin{comment}
    - RA-AAA10 works the best among these
    - RA-AAA10 has a pattern after each 10 m's
    - The same pattern seems to exist for AAA5 and AAA3
    - RA-BRASIL's behave very close to RA-AAA although the poles are different
    - RA-AAAm fails after around m=20, possibly because of the AAA algorithm
    - It gets better for larger lambda??
\end{comment}

\todo[color=pink]{Added \autoref{fig:rationalkrylovpoleselectionbrasil} which compares BRASIL and AAA.}
\todo{Comment on \autoref{fig:rationalkrylovpoleselectionbrasil}.}
\begin{comment}
    - They perform similarly in p=0 but BRASIL outperforms AAA as p grows.
\end{comment}
...
\FloatBarrier
\subsection{Trigonometric Matrix Functions}
\todo[color=pink]{Added this section for the new experiments.}
...
\todo[color=gray]{
    Only AAA1 because the BRASIL seems to be unstable for these matrices.
    Also, AAAn performs the best with n=1.
}
\todo[color=gray]{
...
\todo[color=pink]{
    I also tried for the matrices of the Laplace equation scaled the spectral interval $(\epsilon, 1e+07)$.
    the results were even a bit worse regarding the time step.
}
