...
\end{figure}
\todo[color=pink]{Updated \autoref{fig:scalarphifunctionscomplexplane}: Plots for negative real values.}
...
on the interval $[-1000, 0]$. \autoref{fig:errorsAAA} shows the maximum error of the rational
...
\todo[color=pink]{
    Added \autoref{fig:errorsAAA} which shows that the method good enough and better than
    considering the whole interval.
}
\todo[color=pink]{
    \autoref{fig:errorsAAA} also shows that the errors are smaller for larger $p$'s.
}
...
This hypothesis is also supported by \autoref{fig:errorsAAA}.
